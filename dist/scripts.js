...
export function sampleRUM(checkpoint, data = {}) {
  try {
    ...
    if (!window.hlx.rum) {
      ...
      const isSelected = random * weight < 1; // eslint-disable-next-line object-curly-newline
    ...
    if (random && random * weight < 1) {
      const sendPing = () => {
        ...
        const url = `https://rum.hlx3.page/.rum/${weight}`; // eslint-disable-next-line no-unused-expressions

        navigator.sendBeacon(url, body);
      };

      sendPing(); // special case CWV
...
sampleRUM.observe = elements => {
  elements.forEach(element => {
    if (element.tagName.toLowerCase() === "img" || element.tagName.toLowerCase() === "video" || element.tagName.toLowerCase() === "audio" || element.tagName.toLowerCase() === "iframe") {
...
export function decorateBlocks($main) {
  Array.from($main.querySelectorAll("div.section-wrapper > div > div[class]")).forEach($block => {
    const blockName = $block.className; // $block.classList.add('block');

    $block.setAttribute("data-block-name", blockName);
    const rows = Array.from($block.querySelectorAll(":scope > div"));
    const customEl = rows.reduce((cel, row) => {
...
async function loadBlocks($main) {
  $main.querySelectorAll("div.section-wrapper > div > *[data-block-name]").forEach(async $block => loadBlock($block));
}
/**
...
async function decoratePage(win = window) {
  ...
  if ($main) {
    ...
    setLCPTrigger(doc, async () => {
      loadCSS("/styles/lazy-styles.css"); // post LCP actions go here
