import { createFragmentWithChildren } from './index.js';
import { isCustomElement, setAttribute, areSameTagNames } from './utils.js';
...
class Component extends HTMLElement {
  ...
  _addContent(content) {
    ...
    if (content) {
      const children = Array.isArray(content) ? createFragmentWithChildren(content) : content;
  ...
  _mergeNodes(newChild, existChild, parentNode) {
    if (areSameTagNames(newChild, existChild)) {
      // add new attributes
      this._cloneAttributes(newChild, existChild); // add new props to custom element
  ...
  _mergeCollections(newContent, existChildren, parentNode) {
    if (newContent.length) {
      ...
      for (i = 0, j = 0; i < newContent.length; ++i, ++j) {
        ...
        const newChildPosition = typeof newChild.__DOMPosition__ === 'number' ? newChild.__DOMPosition__ : 0;
        const existChild = existChildren[j];
        const existChildPosition = existChild && typeof existChild.__DOMPosition__ === 'number' ? existChild.__DOMPosition__ : 0;
  ...
  _updateContent(newContent, existChildren = this.childNodes, parentNode = this) {
    ...
    if (!newContent) return parentNode.innerHTML = ''; // newContent is array

    if (Array.isArray(newContent)) return this._updateContent(createFragmentWithChildren(newContent).childNodes, existChildren, parentNode); // newContent is DocumentFragment

    if (newContent.nodeType === 11) return this._updateContent(newContent.childNodes, existChildren, parentNode); // if newContent is already collection then do nothing. if it's not,
    // then newContent is HTMLElement and we must create array with it

    const newCollection = typeof newContent[Symbol.iterator] === 'function' ? [...newContent] : [newContent];
  ...
  setState(newState) {
    if (typeof newState === 'function') {
      const computedState = newState({ ...this.state
      }, { ...this.props
      });
