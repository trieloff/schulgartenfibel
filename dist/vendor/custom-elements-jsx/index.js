...
const fillFragmentByChildren = (fragment, props, child, index) => {
  if (child) {
    if (child instanceof Array) {
      return child.forEach(fillFragmentByChildren.bind(this, fragment, props));
    }
    ...
    if (utils.isCustomElement(child)) child.props = { ...child.props,
      ...props
    };
...
export const createFragmentWithChildren = (children, props = {}) => {
  const fragment = document.createDocumentFragment(); // fill fragment by children
...
const jsx = (tagName, attrs, ...children) => {
  ...
  const element = utils.isSVG(tagName) ? document.createElementNS('http://www.w3.org/2000/svg', tagName) : document.createElement(tagName); // create children fragment and append it to the main element
